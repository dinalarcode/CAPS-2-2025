  ...
  static Future<List<Map<String, dynamic>>> _getMealRecommendations({
    ...
    try {
      ...
      for (var doc in menus.docs) {
        ...
        // Keep image filename as-is from Firebase (will be resolved in UI layer)
...
class MealRecommendationResult {
  ...
  factory MealRecommendationResult.fromMap(Map<String, dynamic> map) {
    // Handle both formats: cache format (direct properties) and original format (dailyStats array)
    double dailyCalories, proteinGrams, carbsGrams, fatsGrams;
    
    if (map.containsKey('dailyStats')) {
      // Original format from MealRecommendationEngine
      final dailyStats = (map['dailyStats'] as List).first as Map<String, dynamic>;
      dailyCalories = (dailyStats['dailyCalories'] as num?)?.toDouble() ?? 0;
      proteinGrams = (dailyStats['protein'] as num?)?.toDouble() ?? 0;
      carbsGrams = (dailyStats['carbs'] as num?)?.toDouble() ?? 0;
      fatsGrams = (dailyStats['fats'] as num?)?.toDouble() ?? 0;
    } else {
      // Cache format (direct properties)
      dailyCalories = (map['dailyCalories'] as num?)?.toDouble() ?? 0;
      proteinGrams = (map['proteinGrams'] as num?)?.toDouble() ?? 0;
      carbsGrams = (map['carbsGrams'] as num?)?.toDouble() ?? 0;
      fatsGrams = (map['fatsGrams'] as num?)?.toDouble() ?? 0;
    }
    
    // Cast lists properly to handle JSON deserialization
    ...
    return MealRecommendationResult(
      sarapan: castList(map['sarapan']),
      makanSiang: castList(map['makanSiang']),
      makanMalam: castList(map['makanMalam']),
      dailyCalories: dailyCalories,
...
final recommendations = await MealRecommendationEngine.getRecommendations(
  ...
  tdee: calculateTDEE(), // dari firebaseService.dart
