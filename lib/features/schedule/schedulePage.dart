                ...
                Row(
                  children: [
                    ...
                    TextButton.icon(
                      ...
                      label: Text(
                        _isToday(selectedDate) ? 'Hari Ini' : 'Pergi ke Hari Ini',
                        style: AppTextStyles.buttonSmall.copyWith(color: AppColors.green),
                      ),
                    ...
                    IconButton(
                      onPressed: () async {
                        ...
                        scaffoldMessenger.showSnackBar(
                          const SnackBar(content: Text('🔄 Re-populating schedules from orders...')),
                        );
                        final success = await ScheduleService.repopulateAllSchedules();
                        if (!mounted) return;
                        scaffoldMessenger.showSnackBar(
                          SnackBar(
                        ...
                        scheduledMeals.asMap().entries.map((entry) {
                          return MealScheduleCard(
                            ...
                            onCheckChanged: (value) {
                              saveChecklistToFirestore(entry.key, value ?? false);
                            },
                          );
                        }),
                      ...
                      children: [
                        ...
                        Text(
                          ...
                          style: AppTextStyles.bodyMedium.copyWith(
                            ...
                            color: isSelected ? AppColors.green : AppColors.lightGreyText,
                          ),
                        ),
                        ...
                        Container(
                          ...
                          height: 48,
                          decoration: BoxDecoration(
                            color: isSelected ? AppColors.green : (isToday ? AppColors.green.withValues(alpha: 0.2) : Colors.transparent),
                            shape: BoxShape.circle,
                            border: isToday && !isSelected ? Border.all(color: AppColors.green, width: 2) : null,
                          ),
                          child: Center(
                            child: Text(
                              '$day',
                              style: AppTextStyles.h3.copyWith(
                                color: isSelected ? AppColors.white : (isToday ? AppColors.green : AppColors.black),
                              ),
                            ),
                          ),
                        ),
...
class MealScheduleCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final fat = parseNutrition(
      meal['fat'] ?? meal['fats'] ?? meal['totalFat']
    );
    
    // Debug: Print actual values to verify
    debugPrint('🍽️ [SCHEDULE] Nutritional data for ${meal['name']}: P=$protein, C=$carbohydrate, F=$fat');
    debugPrint('🕐 [SCHEDULE] Clock field for ${meal['name']}: "${meal['clock']}" (type: ${meal['clock'].runtimeType})');
                    ...
                    children: [
                      ...
                      if ((meal['clock'] ?? '').toString().isNotEmpty)
                        Expanded(
                          child: Row(
                            children: [
                              Icon(Icons.access_time, size: 12, color: AppColors.lightGreyText),
                      ...
                      if (!isFutureDate)
                        Checkbox(
                          ...
                          activeColor: shouldBeAutoChecked ? AppColors.mutedBorderGrey : AppColors.green,
                          checkColor: Colors.white,
                          shape: RoundedRectangleBorder(
                            borderRadius: BorderRadius.circular(4),
                          ),
                          side: BorderSide(
                            color: shouldBeAutoChecked ? AppColors.mutedBorderGrey : AppColors.green,
                            width: 1.5,
                          ),
                  ...
                  RichText(
                    ...
                    text: TextSpan(
                      ...
                      children: [
                        ...
                        TextSpan(
                          text: '(P: ${protein}g | K: ${carbohydrate}g | L: ${fat}g)',
...
class AIFoodLogCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final totalFat = parseNutrition(
      log['fat'] ?? log['fats'] ?? log['totalFat']
    );
              ...
              child: Column(
                ...
                children: [
                  ...
                  ...items.map((item) {
                    ...
                    final itemProtein = parseNutrition(item['protein'] ?? item['proteins']);
                    final itemCarbs = parseNutrition(item['carbohydrate'] ?? item['carbs']);
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Padding(
              padding: const EdgeInsets.symmetric(vertical: AppSpacing.sm * 1.25),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(
